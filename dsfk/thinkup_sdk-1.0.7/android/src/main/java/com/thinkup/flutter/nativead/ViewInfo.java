...
import com.thinkup.flutter.view.IRoundView;
...
//            _info.cornerRadius = Utils.dip2px(FlutterPluginUtil.getActivity(), (int) settingsMap.get(Const.CORNER_RADIUS)) + py;
        ...
        if (settingsMap.containsKey(Const.IMAGE_PTUH)) {
            _info.imagePath = (String) settingsMap.get(Const.IMAGE_PTUH);
...
//                pViewInfo.mView.setBackgroundColor(pViewInfo.bgcolor);
    ...
    public static void addNativeAdView2Activity(final Activity pActivity, final ViewInfo pViewInfo, final TUNativeAdView mTUNativeAdView, final int parentGravity) {

        if (pActivity == null || mTUNativeAdView == null) {
        ...
        pActivity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                try {
                    ViewGroup _viewGroup = (ViewGroup) mTUNativeAdView.getParent();
                    if (_viewGroup != null) {
                        _viewGroup.removeView(mTUNativeAdView);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
...
//                        mTUNativeAdView.setBackgroundColor(Color.parseColor(pViewInfo.rootView.bgcolor));
//                    }

                    MsgTools.printMsg("Add native view to content start....");
                    pActivity.addContentView(mTUNativeAdView, layoutParams);
