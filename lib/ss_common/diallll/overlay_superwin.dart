...
class OverlaySuperwin {
  ...
  void show({
    ...
    SSEventReporttttt.cash_pop(pop_type: "super_win", pop_from: scene.name);
    _overlay = null;
    btnMoney.play();
      ...
      builder: (context) {
        return SuperwinWidget(
          ...
          onBtn2: (double money)async {
            close();
            SSEventReporttttt.cash_pop_close(pop_type: "super_win", pop_from: scene.name);
            bool result = await SSCommonAds().showInterstitialAd(
              adPosId: SSAdsPosId.eyomt_superwin_int,
              ignored_hasDisplayAd: true,
            );
            if(!result){
              money = 0;
